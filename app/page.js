...
const ProfessionalWebsite = () => {
  ...
  const fadeInUp = (id) =>
    `transform transition-all duration-1000 ease-out ${isVisible[id] ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
    }`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 text-gray-900 font-sans overflow-hidden">
      ...
      <section id="hero" className="relative min-h-screen flex items-center justify-center pt-16 overflow-hidden">
        ...
        {/* CrowdCanvas walking figures (Desktop Only) */}
      ...
      <section id="services" className="py-20 bg-gradient-to-br from-gray-50 to-white w-full overflow-hidden">
        <div className="w-full px-0 sm:px-0 lg:px-0">
          ...
          <div className="sm:hidden relative w-full px-2 py-4">
            <div className="flex overflow-x-auto snap-x snap-mandatory space-x-4 pb-6 pt-2 px-4 no-scrollbar">
              {ourServices.map((item, index) => (
                <div
                  key={index}
                  className="snap-center shrink-0 w-[285px] bg-white rounded-3xl p-5 border border-gray-200/90 shadow-xl flex flex-col items-center text-center"
                >
                  <div className="relative w-full h-52 rounded-2xl overflow-hidden mb-4 bg-slate-900 shadow-inner">
                    <img
                      src={item.src}
                      alt={item.name}
          ...
          {/* Desktop View: Interactive 3D Draggable Floating Canvas */}
